...
/// <summary>
/// Backs the Sector dialog's "New" (map-wide) and "Unused" (this element
/// type only) tag-allocation buttons. Only scans what this project's Core
/// actually models today - each sector's own <c>id</c>/<c>moreids</c>
/// fields and each linedef's own <c>id</c> field - not linedef
...
public static class MapDataTagQueries
{
    public static IReadOnlySet<long> GetUsedTags(this MapData map)
    {
        ...
        {
            var id = linedef.Fields.GetInteger("id", 0);
            if (id != 0) used.Add(id);
        }
